...
To ensure the documentation is deployed correctly on GitHub Pages:
1. Go to repository **Settings** > **Pages**.
2. Under **Build and deployment** > **Source**, select **Deploy from a branch**.
3. Under **Branch**, select `gh-pages` and `/ (root)`.
4. Click **Save**.

This ensures GitHub serves the content generated by the workflow (on the `gh-pages` branch) rather than the project's root `README.md`.

## 6. Integration Tests

In addition to unit tests, we maintain a suite of integration tests in `python/integration_tests` to verify the end-to-end functionality of the CLI, Library, and Example Scripts using "Golden Data" (recorded baseline outputs).

### Running Integration Tests
To run the integration tests using the Makefile:
```bash
cd python
make run-integration-tests
```
This runs `pytest integration_tests/`, comparing current outputs against the expected JSON snapshots in `integration_tests/validation_data/`.

### Updating Golden Data
If you make changes that intentionally alter the output format (e.g., adding a new field to the CSV export), you need to update the golden data snapshots.

Run the tests with the `UPDATE_GOLDEN_DATA` environment variable set:
```bash
cd python
UPDATE_GOLDEN_DATA=1 make run-integration-tests
```
This will overwrite the files in `integration_tests/validation_data/` with the outputs from the current run. **Review these changes carefully** before committing to ensure they are correct.
